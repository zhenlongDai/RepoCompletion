...
def get_part_eval_dataset(data_list, local_dp_rank, dp_size):
    ...
    # 获取当前进程需要处理的数据
    part_data_list = copy.deepcopy(data_list[start:end])
    ...
    return part_data_list, part_prompt_list
...
class InferencePipeline:
    ...
    def __init__(self, args):
        ...
        data_list = load_eval_dataset(args.eval_dataset_name, data_path_dir, self.language, self.model_path, self.max_input_tokens, self.debug_mode)
        
        manager = Manager()
        self.data_list = manager.list(data_list)
        self.result_queue = Queue()  # 用于存储每个进程的结果
        #lock = Lock()  # 创建锁
    ...
    def generation(self, data_list, model_path, dp_size, tp_size, local_dp_rank, dp_master_ip, dp_master_port,
                temperature, top_p, gpu_memory_utilization, result_queue, debug_mode = True):
        ...
        for i, output in enumerate(outputs):
            ...
            data = copy.deepcopy(part_eval_dataset[i])
            new_data = {"prompt": data['prompt'], "tag": data['tag'], "id": data['id']}
            new_data['outputs'] = generated_text
            result.append(new_data)
            #part_eval_dataset[i]['outputs'] = generated_text
            #if debug_mode:
        ...
        for local_dp_rank in range(0, self.tp_size):
            proc = Process(target=self.generation,
                            args=(self.data_list, self.model_path, self.dp_size, self.tp_size,
                                  ...
                                  self.result_queue, self.debug_mode)
                            )
            proc.start()
            procs.append(proc)
